...
const cases = [
  { id: 'kulture-market', title: 'Kulture Market', sector: 'E-commerce', desc: 'A global multi-vendor platform connecting local artisans to worldwide markets with automated logistics and FX settlement.', img:'/assets/component1.png' },
];
...
const Portfolio: React.FC<{ onSelectCase: (id: string) => void }> = ({ onSelectCase }) => {
  ...
  return (
    <div className="pt-40 pb-20 relative bg-[#030712]">
      ...
      <section className="px-4 sm:px-6 mb-20 sm:mb-32 mx-auto text-center relative z-10 min-h-[50vh] sm:min-h-[60vh] flex flex-col items-center justify-center">
        <StarField />
        <SVGFlowLines />
        ...
        <div className="relative z-20 space-y-8 sm:space-y-12">
          <div className="inline-block">
            <span className="text-[5px] sm:text-[6px] font-black tracking-[0.3em] sm:tracking-[0.5em] text-blue-500 uppercase px-4 sm:px-6 py-2 rounded-full border border-blue-500/20 bg-blue-500/5 backdrop-blur-sm">
              Portfolio & Impact
            </span>
          </div>
          
          <h1 className="text-3xl sm:text-5xl md:text-6xl lg:text-[9rem] font-extrabold tracking-tighter leading-[0.85] text-white">
            Digital <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-b from-white via-white to-white/20">Masterpieces</span>
          </h1>
          
          <p className="max-w-2xl mx-auto text-sm sm:text-lg md:text-2xl text-gray-400 font-light leading-relaxed">
      ...
      <section className="px-4 sm:px-6 mb-20 sm:mb-32 max-w-7xl mx-auto">
        <div className="relative glass-card rounded-2xl sm:rounded-[4rem] overflow-hidden border-white/5 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.6)]">
          <div className={`flex flex-col lg:flex-row h-full transition-all duration-500 ${isAnimating ? 'opacity-0 translate-y-4 blur-sm' : 'opacity-100 translate-y-0 blur-0'}`}>
            <div className="lg:w-1/2 p-6 sm:p-12 lg:p-24 flex flex-col justify-center">
              <div className="flex items-center space-x-3 mb-6">
                <span className="text-blue-400 text-[10px] sm:text-xs font-black uppercase tracking-[0.2em]">{currentCase.sector}</span>
                <div className="h-[1px] w-8 sm:w-12 bg-blue-500/30"></div>
              </div>
              <h3 className="text-2xl sm:text-4xl lg:text-7xl font-extrabold mb-6 sm:mb-8 tracking-tight">{currentCase.title}</h3>
              <p className="text-gray-400 text-sm sm:text-lg lg:text-2xl font-light leading-relaxed mb-8 sm:mb-12">{currentCase.desc}</p>
              <div className="flex flex-wrap gap-4 sm:gap-6">
                <button onClick={() => onSelectCase(currentCase.id)} className="px-6 sm:px-12 py-3 sm:py-6 bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm sm:text-base rounded-xl sm:rounded-2xl shadow-xl shadow-blue-900/20 active:scale-95 flex items-center transition-all">
                  <span>Explore Build</span> <ArrowUpRight className="ml-2 sm:ml-3" size={18} />
                </button>
              </div>
            </div>
            <div className="lg:w-1/2 relative h-[300px] sm:h-[400px] lg:h-auto overflow-hidden">
              ...
              {/* Overlay graphics */}
              <div className="absolute top-6 sm:top-10 right-6 sm:right-10 p-4 sm:p-6 glass-card rounded-xl sm:rounded-2xl border-white/10 hidden lg:block backdrop-blur-md">
                <p className="text-[8px] sm:text-[10px] font-black uppercase tracking-widest text-gray-500 mb-2">Build Status</p>
                <div className="flex items-center space-x-2">
                  <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
                  <span className="text-xs sm:text-sm font-bold text-white uppercase tracking-tighter">Operational</span>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Controls */}
          <div className="absolute bottom-4 sm:bottom-10 left-4 sm:left-12 lg:left-24 flex items-center space-x-4 sm:space-x-8">
            <div className="flex space-x-2">
              {cases.map((_, i) => (
                <button key={i} onClick={() => setActiveCaseIdx(i)} className={`h-1.5 rounded-full transition-all duration-500 ${activeCaseIdx === i ? 'w-8 sm:w-12 bg-blue-500' : 'w-2 sm:w-3 bg-white/10 hover:bg-white/20'}`} />
              ))}
            </div>
            <div className="flex space-x-2 sm:space-x-4">
              <button onClick={handlePrev} className="w-10 h-10 sm:w-14 sm:h-14 rounded-full glass-card flex items-center justify-center text-white/50 hover:text-white hover:bg-white/5 transition-all"><ChevronLeft size={20} className="sm:w-6 sm:h-6" /></button>
              <button onClick={handleNext} className="w-10 h-10 sm:w-14 sm:h-14 rounded-full glass-card flex items-center justify-center text-white/50 hover:text-white hover:bg-white/5 transition-all"><ChevronRight size={20} className="sm:w-6 sm:h-6" /></button>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* Sectors / Verticals */}
      <section className="py-16 sm:py-24 px-4 sm:px-6 bg-white/[0.02] border-y border-white/5">
        <div className="max-w-7xl mx-auto">
          <div className="mb-12 sm:mb-20">
            <h2 className="text-2xl sm:text-4xl font-extrabold mb-3 sm:mb-4 tracking-tight">Industries of Impact</h2>
            <p className="text-gray-400 font-light text-sm sm:text-base">Specialized expertise in the verticals defining the modern economy.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-8">
            {sectors.map((sector, i) => (
              <div key={i} className="glass-card p-6 sm:p-12 rounded-2xl sm:rounded-[3.5rem] border-white/5 hover:border-blue-500/20 transition-all group">
                <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-lg sm:rounded-2xl bg-white/5 flex items-center justify-center mb-6 sm:mb-8 group-hover:scale-110 transition-transform">{sector.icon}</div>
                <h4 className="text-lg sm:text-2xl font-bold mb-3 sm:mb-4">{sector.name}</h4>
                <p className="text-gray-500 leading-relaxed font-light text-sm sm:text-base">{sector.desc}</p>
              </div>
            ))}
          </div>
        </div>
...
{/* 
      ...
      <section className="py-16 sm:py-32 px-4 sm:px-6 text-center max-w-4xl mx-auto relative overflow-hidden">
        ...
        <h2 className="text-2xl sm:text-4xl md:text-6xl lg:text-7xl font-extrabold mb-8 sm:mb-12 tracking-tight leading-tight">Build Your Product <br className="hidden md:block" /> with the Best.</h2>
        <div className="flex flex-col sm:flex-row justify-center items-center gap-4 sm:gap-6">
          <button className="w-full sm:w-auto px-8 sm:px-14 py-4 sm:py-7 bg-blue-600 hover:bg-blue-500 text-white font-bold sm:font-black text-base sm:text-lg md:text-xl rounded-xl sm:rounded-2xl shadow-2xl active:scale-95 transition-all">Start Project</button>
          <button className="w-full sm:w-auto px-8 sm:px-14 py-4 sm:py-7 glass-card hover:bg-white/5 border border-white/10 text-white font-bold text-base sm:text-lg md:text-xl rounded-xl sm:rounded-2xl active:scale-95 transition-all">Talk to Engineering</button>
        </div>
      </section>
    </div>
  );
